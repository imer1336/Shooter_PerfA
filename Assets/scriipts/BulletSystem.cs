    ...
    private void OnUpdate(ref SystemState state)
    {
        ...
        foreach (Entity entity in allEntities)
        {
            if (entityManager.HasComponent<BulletComponent>(entity))
            {
                ...
                physicWorld.SphereCastAll(bulletTransform.Position, bulletComponent.Size / 2, float3.zero, 1,
                    ref hits,
                    new CollisionFilter
                        { BelongsTo = (uint)CollisionLayer.Default, CollidesWith = (uint)CollisionLayer.Enemy });
